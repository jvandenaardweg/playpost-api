...
export const getArticleIdFromUrl = (url: string) => {
  ...
  if (possibleId.length !== 12) {
    throw new Error(`Could not get the correct Medium Post ID from URL '${url}'.`);
  }
...
export const getArticle = (url: string) => {
  ...
  return fetch(`${url}?format=json`)
    ...
    .then((text: string) => JSON.parse(text.split('</x>')[1]).payload)
    .then((json: any) => {
      const firstAuthor: Author = (json.references.User) ? Object.values(json.references.User)[0] : {};
      const firstCollection: Collection = (json.references.Collection) ? Object.values(json.references.Collection)[0] : {};
...
export const getArticleById = (mediumPostId: string) => {
