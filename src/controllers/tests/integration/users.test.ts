...
describe('users', () => {
  ...
  describe('authenticated integration test', () => {
    ...
    it('Authenticated users should have access to the POST /users endpoint.', async () => {
      const response = await nodeFetch('http://localhost:3000/v1/users', { headers, method: 'POST' });
      expect(response.status).not.toBe(403);
    });

    it('Authenticated users do not have access to the GET /users endpoint.', async () => {
      const response = await nodeFetch('http://localhost:3000/v1/users', { headers, method: 'GET' });
      expect(response.status).toBe(403);
    });

    it('Authenticated users do not have access to the DELETE /users/:userId endpoint.', async () => {
      const response = await nodeFetch('http://localhost:3000/v1/users/93f43c1b-aad3-406e-ad34-110ef77573da', { headers, method: 'DELETE' });
      expect(response.status).toBe(403);
    });
